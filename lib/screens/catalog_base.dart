...
/// Lógica de datos y filas COMPARTIDAS del Inicio entre las variantes touch
/// (móvil/tablet) y TV. Cada variante aporta su propio hero y chrome mediante
/// [_buildHero]/[_showLogo]. La privacidad `_` funciona entre estas partes
/// porque comparten la misma librería (`catalog_screen.dart`).
mixin CatalogBaseState<T extends StatefulWidget> on State<T> {
  final _store = ContentStore.instance;

  /// Hero específico del dispositivo (billboard en TV, banner rotativo en touch).
  Widget _buildHero();

  /// Solo touch muestra el logo en la barra superior; en TV lo pone el riel.
  bool get _showLogo;

  /// Punto de enganche para que la variante TV mueva el billboard al foco.
  /// No-op en touch.
  void _onPosterFocus(Channel channel, bool focused) {}

  /// Candidatas al hero: con logo, destacadas primero.
  List<Channel> get _heroMovies {
  ...
  Widget _content() {
    ...
    return ListView(
      ...
      children: [
        if (recommended || moviesTab) _buildHero(),
