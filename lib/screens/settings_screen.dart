...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  void initState() {
    super.initState();
    _sortBy = StorageService.getSetting('sortBy', defaultValue: 'name') as String;
    _autoPlay = StorageService.getSetting('autoPlay', defaultValue: true) == true;
    _forceLandscape = StorageService.getSetting('forceLandscape', defaultValue: false) == true;
    _wifiOnly = StorageService.getSetting('wifiOnly', defaultValue: false) == true;
  }
          ...
          _section('Reproducción', [
            ...
            _toggle(
              ...
              onChanged: (v) async {
                ...
                await _set('forceLandscape', v);
              },
            ),
          ]),
          _section('Red y datos', [
            _toggle(
              icon: Icons.wifi,
              title: 'Solo por WiFi',
  ...
  Widget _header() => Column(children: [
    Padding(
      ...
      child: Row(children: [
        ...
        const Text('Ajustes', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: AppColors.textPrimary, letterSpacing: -0.3)),
      ]),
    ),
    // Tarjeta de marca
    Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: AppColors.accent.withValues(alpha: 0.28), blurRadius: 18, offset: const Offset(0, 8))],
      ),
      child: Row(children: [
        Container(
          width: 50, height: 50,
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: const [
          Text('HourTV', style: TextStyle(color: Colors.white, fontSize: 19, fontWeight: FontWeight.w800)),
          SizedBox(height: 2),
          Text('Versión 1.0.0 · IPTV personal', style: TextStyle(color: Colors.white70, fontSize: 12.5)),
        ]),
      ]),
    ),
  ]);
  ...
  Widget _section(String title, List<Widget> tiles) => Column(
    ...
    children: [
      Padding(
        ...
        child: Text(title.toUpperCase(), style: const TextStyle(color: AppColors.textMuted, fontSize: 12, fontWeight: FontWeight.w700, letterSpacing: 1.2)),
      ),
      Container(
  ...
  List<Widget> _withDividers(List<Widget> tiles) {
    ...
    for (var i = 0; i < tiles.length; i++) {
      out.add(tiles[i]);
      if (i < tiles.length - 1) {
        out.add(Divider(height: 1, thickness: 1, indent: 62, color: Colors.white.withValues(alpha: 0.05)));
      }
    }
    return out;
  }
  ...
  Widget _toggle({required IconData icon, required String title, required String subtitle, required bool value, required ValueChanged<bool> onChanged}) {
    return ListTile(
      ...
      title: Text(title, style: const TextStyle(color: AppColors.textPrimary, fontSize: 14.5, fontWeight: FontWeight.w600)),
      subtitle: Text(subtitle, style: const TextStyle(color: AppColors.textMuted, fontSize: 12)),
      trailing: Switch(value: value, onChanged: onChanged, activeThumbColor: AppColors.accent, activeTrackColor: AppColors.accent.withValues(alpha: 0.5)),
      onTap: () => onChanged(!value),
    );
  }

  Widget _choice({required IconData icon, required String title, required String subtitle, required VoidCallback onTap}) {
    return ListTile(
      contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 2),
      leading: _iconChip(icon),
      title: Text(title, style: const TextStyle(color: AppColors.textPrimary, fontSize: 14.5, fontWeight: FontWeight.w600)),
  ...
  Future<void> _showSortDialog() async {
    final choice = await showDialog<String>(
      ...
      builder: (ctx) => SimpleDialog(
        ...
        title: const Text('Ordenar canales por', style: TextStyle(color: AppColors.textPrimary)),
        children: [
  ...
  Widget _dialogOption(BuildContext ctx, String value, String label) {
    ...
    return SimpleDialogOption(
      ...
      child: Row(children: [
        Icon(selected ? Icons.radio_button_checked : Icons.radio_button_unchecked, color: AppColors.accent, size: 20),
        const SizedBox(width: 12),
        Text(label, style: TextStyle(color: selected ? AppColors.accent : AppColors.textPrimary, fontSize: 14)),
      ]),
    );
  }
  ...
  Future<void> _clearCache() async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: AppColors.surfaceDark,
        title: const Text('Limpiar caché', style: TextStyle(color: AppColors.textPrimary)),
        content: const Text('Se eliminarán las imágenes descargadas y el historial de canales recientes.', style: TextStyle(color: AppColors.textSecondary)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancelar')),
          ElevatedButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('Limpiar')),
        ],
      ),
    );
    if (ok != true) return;
  ...
  void _showAbout() {
    showAboutDialog(
      ...
      applicationIcon: const Icon(Icons.live_tv, color: AppColors.accent, size: 40),
      children: const [
        Text('App IPTV personal para Android, Android TV, Google TV y PC.\n\nSoporta listas M3U estándar.', style: TextStyle(color: AppColors.textSecondary)),
      ],
    );
  }
}
